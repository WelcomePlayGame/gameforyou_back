...
@Slf4j
public class GamePostDesUrlServicePL implements ForkWithFile {
    ...
    final private GamePostDesUrlRepositoryPL repository;

    public List<GamePost_des_urlsPL> getListDesurl() {
        return repository.findAll();
    }

    @Transactional
    public GamePost_des_urlsPL  addGamePostDesUrl(MultipartFile photo) throws IOException {
        String namePhoto = generateNameFile(photo);
        log.info(UPLOAD_CATALOG);
        createDirectory(UPLOAD_CATALOG, namePhoto, photo);
        GamePost_des_urlsPL gamePost_des_urls =  new GamePost_des_urlsPL();
